...
class Sage:
    def __init__(self, story):
        self.story = story
        np = Player(story, "N/A", 1) # new instance of player, np. Will change after next update probably.
        self.play(story, np)
    ...
    def play(self, story, np, current_room="main"):
        ...
        choice = input("Choice: ") #everything in this game hinges on this input...
        if choice in (story["story"]["rooms"][current_room]["available_dir"]): #if player types direction, looks in json and prints message if it's there
            ...
            if (story["story"]["rooms"][next_room]["obstacles"]) == None: #checks for an obstacle
                ...
                if (story["story"]["rooms"][current_room]["NPCs"]) != None:
                    ...
                    if (story["story"]["NPCs"][npc]["is_enemy"]) == True:
                        enemy = Enemy(story, npc, np)
        ...
        #Could I possibly make everthing below this comment a match (choice): ?
        #maybe, I'll have to look into it.
        elif choice == "quit" or choice == "exit": #exit case
            print("Quitting...\n")

        elif choice == "look": #find items/people to talk to
            ...
            try: #checks to see if 'NPCs' is a field in JSON
                if (story["story"]["rooms"][current_room]["NPCs"]) == None:
                    ...
                    npc = (story["story"]["rooms"][current_room]["NPCs"])
                    chatterbox = NPC(story, npc, np)
            except: #if error, just assume there is no person
                print("\nThere is nobody to talk to!\n")
            finally:
                self.play(story, np, current_room)

        elif choice == 'say': #just a silly little function. Might come in handy later.
            ...
            if (secret == 1):
                print("\nDon't be silly.\nJust kidding, try asking again!\n")
                self.play(story, np, current_room)
            else:
                print("\n"+story["story"]["help"]+"\n")
                self.play(story, np, current_room)

        elif choice == "cheat": #debug option that gives whatever
            print("\nOK cheater.\nWhat do you want to do?\n")
            cheat = input("Cheat: ")
            match cheat:
                case "give":
                ...
                case "teleport":
                    ...
                    room = input("Room: ")
                    if room in (story["story"]["rooms"]):
                        current_room = room
                case "_":
                    print("I don't know that cheat...")
            self.play(story, np, current_room)
